---
name: brainstorm
description: "将用户的高层级想法转化为完整设计规格。在创建功能、构建组件、添加新能力或修改现有行为之前触发，通过提问、探索方案、展示设计、获得批准并撰写设计文档。"
---

# 将想法头脑风暴为设计

通过自然的协作对话帮助用户将想法转化为完整的设计规格，然后再进入实施阶段。
...
3. **提出 2-3 种方案** — 附带权衡分析和你推荐的方案
4. **展示设计** — 按复杂度分节展示，每节之后获得用户确认
5. **撰写设计文档** — 保存到 `docs/yo/specs/YYYY-MM-DD-<topic>-design.md` 并提交
6. **规格自检** — 快速内联检查占位符、矛盾、歧义、范围以及用户意图匹配度
7. **用户审阅书面规格** — 请用户在继续之前审阅规格文件
...
5. **用户意图匹配度：** 当前规格是否满足用户最初的目的、约束和成功标准？有没有遗漏的需求？
...
- **不要获得批准前就实施** — 即使是"两行代码"的改动，也必须先展示设计并让用户批准。
- **不要一次抛多个问题** — 一条消息只问一个澄清问题，复杂的议题拆成多轮。
- **不要跳过上下文检查** — 如果不了解当前项目状态就开始设计，容易重复已有模式或遗漏关键约束。
- **不要把规格写得比需要的更大** — 如果项目明显超出单个规格的范围，先帮助用户拆分，不要硬塞。
- **不要替换用户的路径偏好** — 默认规格路径是 `docs/yo/specs/...`，但用户如果指定了其他位置，优先使用用户的。
