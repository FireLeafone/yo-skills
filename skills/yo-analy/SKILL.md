...
| **explaining-project** | `reference/explaining-project/` | 分析代码库并生成详尽的人读项目文档 `explaining-project.md` | "分析这个项目"、"新人怎么上手"、"项目文档" |
...
**理解与文档**：
- **explaining-project** → 新人上手、人读项目文档（`explaining-project.md`）
...
- `/yo-analy explaining-project ...` → 加载 `reference/explaining-project/SKILL.md`
- `/yo-analy agents-docs ...` → 加载 `reference/agents-docs/SKILL.md`
...
| 分析项目、项目文档、新人上手、项目结构、生成文档、文档化、项目解析 | **explaining-project** |
...
常见组合建议：
- "分析项目并严格审查方案" → 先 **explaining-project** / **agents-docs**，再 **grill-me**
- "讲完代码后总结给下个 agent" → 先 **explain-code**，再 **extract-chat**
- "设计审查对话完要总结" → 先 **grill-me**，再 **extract-chat**
- "理解项目后写总结文档给agent" → 先 **explaining-project** / **agents-docs**，再 **extract-chat**
- "讲个故事帮我理解这个设计模式" → **tell-story**
- "理解概念后深入代码实现" → 先 **tell-story**，再 **explain-code**
- "用最简单的话讲给完全不懂的人，要图片/草图" → **tell-me**

**grill-me 与 explain-code / explaining-project 歧义时：** 用户要被动理解现有代码或项目 → **explain-code** / **explaining-project**；用户要主动被追问、拍板方案 → **grill-me**。

**extract-chat 与 explaining-project / agents-docs 歧义时：** 用户要生成面向仓库的长期协作文档（AGENTS.md 等）→ **agents-docs**；用户要生成人读项目文档 → **explaining-project**；用户要沉淀当前会话、供下一位 agent 接续 → **extract-chat**。
