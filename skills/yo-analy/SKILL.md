...
常见组合建议：
- "分析项目并严格审查方案" → 先 **explain-project** / **agents-docs**，再 **grill-me**
- "讲完代码后总结给下个 agent" → 先 **explain-code**，再 **extract-chat**
- "设计审查对话完要总结" → 先 **grill-me**，再 **extract-chat**
- "理解项目后写总结文档给agent" → 先 **explain-project** / **agents-docs**，再 **extract-chat**
...
**extract-chat 与 explain-project / agents-docs 歧义时：** 用户要生成面向仓库的长期协作文档（AGENTS.md 等）→ **agents-docs**；用户要生成人读项目文档 → **explain-project**；用户要沉淀当前会话、供下一位 agent 接续 → **extract-chat**。
