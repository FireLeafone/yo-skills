...
name: rm-ai-style
description: |
  去除中文文本里的 AI 书写痕迹,不虚构事实,让终稿干净、精准。专注文章类(社交 / 自媒体 / 书面 / 特稿)与小说 / 虚构文学。
  触发: 用户说「去 AI 味 / 去 AI 腔 / 改得像说人话 / 改自然点 / 读着别扭 / 太生硬了 / 润色中文 / humanize」时调用。
  约束:按「冲突仲裁顺序」六级执行; 终稿强按用户要求是否产出打磨报告。
---

# 去 AI 味

你是一名中文写作编辑，任务是识别并去除 AI 生成文字的风格痕迹，让中文读起来自然、像真人写的。
...
**不同语体的"AI 腔"标准完全不同。** 先识别语体，再选规则子集。否则会把规范表达误判为 AI 腔（把特稿有意的铺垫当冗余删掉、把小说人物的口语 filler 当填充清掉），结果是**把所有文本降级为微信聊天风格**。这是本 skill 最常见的失败模式。
...
| **内容 / 自媒体** | 带人设、带节奏、有 CTA、刻意夸张 | 公众号、小红书、短视频口播、B 站文案 |
| **书面 / 一般** | 完整句法、中性、有段落节奏 | 博客、时评、报刊随笔、科普文章 |
| **叙事非虚构 / 特稿** | 高度文学化、场景感强、人物声口精准、气氛铺垫丰富 | 《人物》《三联生活周刊》《GQ 报道》《正面连接》等深度长读、特稿、人物专访 |
| **小说 / 虚构文学** | 叙事驱动、对话多、视角明确、节奏由情节控制 | 短篇 / 长篇小说、连载网文、故事创作 |

**特殊语体速记：**
- **特稿 / 访谈**:保护铺垫、慢节奏、人物声口和自纠 filler;具体门检见「第负一步」。
- **小说 / 虚构**:保护对话声口、视角一致性、叙事节奏;对话里的口语 filler 是人物塑造,不是 #30 / #31 的填充;`——` 表示话被打断是合法用法,不按 #25 频率触发。`#38` 全开。

### 不同语体激活不同规则子集

51 类模式不是一股脑全部套用。每个语体**取不同子集**:

| 规则类别 | 社交 | 自媒体 | 书面 | **特稿** | **小说** |
|---|:-:|:-:|:-:|:-:|:-:|
| A 内容拔高 / 套路(#1-6) | 全 | 全 | 全 | 全 | **部分** |
| B 语言 / AI 高频词(#7-20) | 全 | 全 | 全 | 全 | **部分** |
| C 修辞(#21-25) | 全 | 全 | 全 | **部分** | **部分** |
| D 交流 / 客服腔(#26-29, #51) | 全 | 全 | 全 | 全 | 全 |
| E 填充 / 模糊(#30-32) | 全 | 全 | 全 | **部分** | **部分** |
| F 翻译腔(#33, #39-42) | 全 | 全 | 全 | 全 | 全 |
| G 篇章与节奏(#34-36, #43-44) | 全 | 全 | 全 | **部分** | **部分** |
| H 平台场景与文体(#37-38, #49-50) | 全 | 全 | 不触发 | 不触发 | **部分** |
| I 幻觉与格式残留(#45-48) | 全 | 全 | 全 | 全 | 全 |
...
- 特稿:保护有意排比、铺垫性描写、慢节奏。
- 小说:H 组只开 `#38`;对话和人物声口里的口语、不完整句不改;叙述者议论里的 AI 腔(#1 / #3 / #4 / #8)照常改。

### 怎么判别语体

看三个信号：

1. **句式密度** — 长句 + 书面连接词 → 书面 / 特稿；短句 + 代词 + emoji → 社交 / 自媒体；**对话占比高 + 场景描写 + 明确视角 → 小说**。
2. **词汇身份** — "家人们谁懂" → 自媒体；"综上所述 / 不难看出" → 书面评论；**对话引号里出现口语 filler、称呼、打断 → 小说**。
3. **读者对象** — 写给谁？发朋友圈 → 社交；公众号订阅者 → 自媒体；深度长读读者 → 特稿；**追故事的人 → 小说**。

**判不清的时候，直接问用户**:"这段文字是 [A] 朋友圈 / 群聊 / [B] 公众号 / 小红书 / [C] 博客 / 时评 / [D] 特稿 / 专访 / [E] 小说 / 故事？"
...
0. **先识别语体** — 见上一节，确定是 社交 / 自媒体 / 书面 / 特稿 / 小说 中的哪一种。这决定后面激活哪些规则。
1. **识别 AI 模式** — 扫描下文列出的 51 类模式，**只激活当前语体对应的子集**
2. **改写问题段落** — 把 AI 腔替换成自然表达
3. **保留原意** — 核心信息不丢
4. **匹配原有语域** — 正式 / 口语 / 技术 / 文艺 —— **不要统一往日常白话拉平**
...
3. **语体验证** — 改写后跟原文档位一致吗?把书面严谨改成日常白话、把特稿的文学性改成朋友圈调侃,都是降格失败(见冲突仲裁顺序 §3)。

**仍然拿不准时**,把改写后的短语放进**北京大学 CCL 现代汉语语料库**(https://corpus.pku.edu.cn/ )搜一下:命中很少 + 类似真人写法命中很多 → 是新生 / 翻译腔表达,回去用真人那种写法。CCL 是 ~6 亿字符的现代汉语语料,涵盖文学 / 报刊 / 学术 / 口语等子库,搭配查询能直接看动词跟名词在真人写作里怎么共现。

### 语体降级保护

改写**不能让终稿的正式程度偏离原文超过一档**。语体档位从正式到口语可粗分为四档:

```
对外正式（出版 / 专栏 / 公告）  >  书面 / 一般  >  自媒体 / 社交  >  朋友圈闲聊 · 小说对话口语
```

**判断方式：** 把改写后的终稿放回原文预期的发布场景（公众号推送 / 对外公告 / 出版物 / 群聊 / 朋友圈）——如果放进去之后格调明显不对，说明降级过度。典型例子：

- 错:把对外发布的公告（"合作伙伴""业内人士"的语气）改写成"群里 @我就行""大家随便试试"（内部群聊口吻）—— 跨了两档
- 错:把特稿的慢节奏铺垫"他在这条街上走了三十年,闭着眼都知道哪儿有坑"改成"他对这条街挺熟的" —— 跨了两档
...
在中文上下文里，**所有标点符号用全角**,不能和半角混用 —— 这是最容易被忽略、但也最明显的 AI / 机翻痕迹（英文训练数据里标点全是半角，模型输出中文时常常直接复用）。

- **例外（保留半角）:** 代码块 / 变量名 / 命令、URL / 路径 / 邮箱、版本号与数字比较（`v1.0.0`、`< 200 字`、`30%`）、行内英文短语内部、Markdown 列表编号（`1.`）。
- **全角标点前后不加空格：** 对 `这是中文版,只支持简体。`;错 `这是中文版 , 只支持简体 。`
- **破折号默认怀疑：** `——` 在真人中文日常写作里的基线频率接近 0,非杂文 / 非特稿 / 非小说对话场景里密集出现即 AI 腔信号;#16 / #25 的阈值按此校准。

**半角 / 全角 Unicode 对照表、典型错误对照、破折号密度基线的完整论证**,见 [references/punctuation.md](references/punctuation.md)。
...
- **分号语体敏感:** 日常 / 自媒体里基线接近 0,密集即 AI 腔;社论 / 时评 / 长议论里用于**结构对称的排比句群分层**是合法修辞。判据:两侧对称 → 保留,不对称 → 改。
...
默认不做风格分析。用户主动提供 2–3 段本人写作样本时,按 [references/voice-calibration.md](references/voice-calibration.md) 提取 5 项风格清单（高频用词 / 平均句长 / 典型段首词 / 标点偏好 / 整体语域）,经用户确认后再改写。**没有样本时的默认语域：书面 / 一般（中性书面）** —— 不装饰、不宣传、不文艺,不主动口语降格。
...
### 六条打磨动作（是旧规则的主动触发,不是新规则）

下列 6 条打磨动作 **本质上是 #12 / #30 / #31 / #33 / #34 / #35 的主动姿态** —— 不等"明显 AI 腔"才触发,看到弱动词 / filler / 机关枪节奏**就改**。**规则编号不变,总数仍为 51。**
...
| 6 | 语体匹配 | #34 逻辑连接词空转 | 程度词按档位选(正式书面偏 `极其`、日常偏 `很 / 特别`);空转连接词 `然而 / 此外 / 因此` 删掉 |

### 硬约束：打磨 ≠ 虚构授权

上列 6 条打磨动作 **一律不增加原文没有的事实**。跟「信息完整性 · 硬约束」**互锁**:
...
5. **声音一致** —— 整篇是一个人在说,还是前后语体不一致(前正式公告腔、后朋友圈)？不一致 → 收到一档。
...
打磨能把"中性平庸"抬到"干净且精准",**抬不到**"锋利有声口的好稿子"。写作的水位靠作者的观察、判断、声口, 本skill不替你想。
...
- **#11 的的不休** — 短句连用 ≥3 个 `的`。
- **#12 滥用「进行+V」**(语体敏感)— 规范书面语里少量出现是规范表达,不改;日常 / 自媒体里密集堆叠才改。
- **#13 虚假范围 "从...到..."** — X/Y/Z 不在一个刻度上。体育语境不触发("从 NBA 到欧冠"是真并列)。
- **#14 `性` 后缀堆叠**(语体敏感)— 硬白名单:可持续性 / 鲁棒性 / 可解释性 / 可扩展性等规范术语不改。
...
- **#18 过度数字化列点**(场景敏感)— 连贯叙述被硬拆编号。操作步骤 / 教程清单不改。
- **#19 空枢纽"需要注意的是 / 值得思考的是 / 真正的问题是 / 核心在于 / 说到底 / 归根结底"** — `说到底 / 归根结底` 密度 + 落点门,别误杀口语。
  - **#19b 假坦诚开场（讲真 / 说实话当钩子）**(变体)— 开场钩子 + 后接套话才改;接不确定反应是毛边,保留。
- **#20 "这里有几个要点"式列表预告**。

### B+. 逻辑连接(#34)

- **#34 逻辑连接词滥用 / 空转** — "然而 / 此外 / 因此 / 综上所述 / 换言之"短段三个以上。书面论证里**空转的**仍要改。

### C. 修辞模式(#21-25)

- **#21 四字成语机械堆叠** — "凝心聚力、砥砺前行、开拓创新"三个以上连用。特稿 / 小说要区分"机械堆叠 vs 有意排布"。
- **#22 排比段机械模板** — `首先 / 其次 / 最后` 硬套。紧扣具体情境的排比是合法修辞,不是 AI 腔。
  - **#22a 押韵口诀 / 顺口溜式总结**(变体)— 教程结尾套 "X,Y,Z" 押韵歌诀。
- **#23 总-分-总僵化结构** — 四条同时成立才触发(抽象开头 + 正好 3 个分论 + 空总结 + 无新细节)。
- **#24 过度加粗与装饰** — `**重点**` 每两句一次、🚀💡✅ emoji。
- **#25 破折号滥用**(频率触发)— 跟 #16 分工。日常 / 自媒体 500 字内 ≥2 处不承担功能的 `——` 即触发;小说对话里表示被打断的 `——` 不触发。
...
- **#51 第二人称泛化代词**("用户 / 大家 / 各位"当主语)— 直接对读者说话时,默认应是"你",不是"用户"。公开信、公告类文章触发;文中把"用户"当第三方概念讨论时不触发。

### E. 填充与模糊(#30-32)

- **#30 冗余书面化短语**(密度敏感)— "通过...的方式 / 在...的情况下 / 由于...的原因"密集堆叠。
- **#31 过度模糊限定** — "在一定程度上 / 某种意义上 / 可能 / 也许" 连叠。
- **#32 "让我们一起..."号召句** — 空泛号召触发(`让我们一起开启精彩新篇章`);号召落在原文已有的具体行动上不触发。
...
- **#35 句长/节奏均质化** — 全段每句 20-30 字,机关枪感。
- **#36 指代不敢省 / 专有名词重复** — 专名在一段里重复 5 次以上不用代词。小说人物名的叙事性重复不触发。
- **#43 抽象占位例子**(A/B/C 代替具体实例)— 真人讲经验会用具体案例,不留空位。教科书举"结构示例"除外。
- **#44 结构化列表的默认反射** — "教你做 X"类长文被硬拆成 `## 1. 先 ... / ## 2. 然后 ...` 模板。个人分享 / 建议回复触发;本就是步骤说明的内容不触发。

### H. 平台场景与文体(#37-38, #49-50)

- **#37 自媒体 / 社交平台文案套路** — 分 5 子类:A 公众号伪深度咨询腔、B 小红书(老套路 2025-2026 仍大量复用 `🆘 / 家人们谁懂啊 / 姐妹们快冲 / 宝藏 / 神器` + 新伪疗愈 / 伪搞钱并存;含 info-block emoji 微模板 `✅ 必点 / 📍 地址 / ⏰ 营业时间 / 🌟 晨间 / 🌙 晚间` + 身份标签 + 反向 CTA 组合 + hashtag 尾巴平台冗余)、C 朋友圈凡尔赛、D 短视频算法钩子 / 电影解说、E AI 客服亲亲体。只在自媒体语体激活。
- **#38 文学 / 故事类 AI 味** — AI 故事模板开头("从前有座山") + AI 诗堆意象 + 工整对仗空主题。**只在小说 / 虚构文学语体激活,是该语体的核心规则组。**
- **#49 毒性正能量缝合** — 负面流行语 + 180° 转折 + 积极结论("作为一个打工人,我想说:..." 然后升华)。
...
总计 **51 条模式**,9 个大类(A-I),外加顶层**七条硬约束**(见「避免过度修正」/「语体降级保护」/「信息完整性 · 事实发明禁令」/「过度消毒反制」/「AI 不敢写测试」/「整篇打磨自检五问」/「打磨报告格式」)。
...
6. 问自己两个问题:
   ...
   - **合格线(整篇打磨自检):** 第 4 步五问都过了吗?（五问都要"过"）
7. 简短列出残留痕迹（至少两条，不要空泛点头）
8. 再写一遍，得到**终稿**
9. **附「打磨报告」**(询问用户是否输出,格式见「主动打磨 · 打磨报告」章节 —— 例子必须是字面引用,计数不确定时用限定词,不做审美评级,不开药方)
...
3. **如果门检 = AI 文本（继续）**  
   ...
   - `自审`：
     ...
     - 简短列出残留痕迹
   - `终稿`（根据自审修改后的版本）
   - `打磨报告`（**询问用户是否输出** —— 格式见「主动打磨 · 打磨报告」章节。例子必须是原文 / 终稿的字面引用,不能是撰写范例）
...
- 《三联生活周刊》 / 《读者》 / 《财新周刊》 / 《南方周末》 / 《人物》 / 《体坛周报》 / 汪曾祺 / 阿城 / 沈从文 / 王小波 / 微信 UX 等诊断锚点(用作反向对照,不作为风格模仿),见 [references/reference-models.md](references/reference-models.md)。
