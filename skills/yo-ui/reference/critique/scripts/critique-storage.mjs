 ...
 *
 * Each run of /yo-ui critique writes a per-target snapshot to
 *   docs/yo-ui/critique/<YYYY-MM-DD>__<slug>.md
 * with a small YAML frontmatter carrying the score + P0/P1 counts.
 *
 * /yo-ui polish reads the latest matching snapshot at start as its
...
import { getCritiqueDir } from './yo-ui-paths.mjs';
...
/**
 * YYYY-MM-DD date string for snapshot filenames.
 */
export function nowFilenameStamp(date = new Date()) {
  return date.toISOString().slice(0, 10);   // 2026-05-12
}
...
export function writeSnapshot({ slug, meta, body, cwd = process.cwd(), now = new Date() }) {
  ...
  // YO_UI_CRITIQUE_META env var) could clobber them, leaving the
...
function main(argv) {
  ...
  switch (cmd) {
    ...
    case 'write': {
      ...
      let meta = {};
      const metaArg = process.env.YO_UI_CRITIQUE_META;
      if (metaArg) {
